...
## 개선방향
스케치 이미지 분류 모델의 **accuracy**를 높이기 위해 고려할 수 있는 여러 가지 방법이 있습니다. 스케치 이미지 데이터는 일반 사진과 다르게 선이나 윤곽선만으로 표현된 추상적인 이미지이기 때문에 모델이 이러한 특징을 잘 학습하도록 도와야 합니다. 아래에 모델 성능을 높일 수 있는 다양한 방법을 제시합니다.

### 1. **데이터 증강(Data Augmentation)**
   - 스케치 이미지는 다양한 스타일과 각도로 표현될 수 있으므로, **데이터 증강**을 통해 모델이 다양한 상황을 학습할 수 있도록 해야 합니다.
     - **Random Rotation**: 이미지를 다양한 각도로 회전시켜 모델이 방향에 구애받지 않도록 학습시킵니다.
     - **Random Scaling and Cropping**: 스케치 이미지의 크기나 일부분을 다르게 하여 모델이 이미지의 부분적인 특성을 학습하도록 합니다.
     - **Random Flip**: 좌우 반전 같은 데이터 증강을 통해 다양한 표현을 학습할 수 있습니다.
...
### 2. **모델 아키텍처 개선**
   - 스케치 이미지는 색상이나 질감 정보가 없고 형태만으로 구성되어 있기 때문에, **Convolutional Neural Networks (CNN)**과 같은 모델이 효과적입니다. 특히, 더 복잡한 아키텍처를 사용하거나 **Transfer Learning**을 통해 ImageNet과 같은 대형 데이터셋에서 미리 학습된 모델을 사용하는 것이 유리합니다.
     - **ResNet, EfficientNet** 같은 대형 모델을 사용한 Transfer Learning.
     - 미리 학습된 모델을 사용하고, 마지막 레이어만 Fine-Tuning합니다.
...
### 3. **Progressive Resizing 사용**
   - Progressive Resizing은 초기 학습에서 작은 해상도의 이미지를 사용하여 빠르게 학습하고, 점차 해상도를 높여가는 기법입니다. 이를 통해 처음에는 빠르게 패턴을 학습하고 이후에는 더 세밀한 패턴을 학습할 수 있습니다.

### 4. **Batch Normalization 및 Dropout**
   - **Batch Normalization**은 모델이 빠르게 수렴하도록 도와주며, **Dropout**은 과적합을 방지하여 성능을 높일 수 있습니다. 네트워크 중간에 적절히 Dropout을 추가하는 것도 중요합니다.
...
### 5. **앙상블(Ensemble) 기법**
   - 여러 모델의 예측 결과를 결합하여 최종 출력을 계산하는 앙상블 기법을 사용하면 성능을 향상시킬 수 있습니다. 다양한 모델을 학습시키고 그 결과를 합산 또는 가중 평균을 내어 최종 예측값을 결정합니다.

### 6. **학습률 조정(Learning Rate Scheduling)**
   - **Learning Rate Scheduler**를 사용하여 학습이 진행됨에 따라 학습률을 점차 줄여가며 최적화 과정을 안정화시킵니다. 이를 통해 모델이 안정적으로 수렴할 수 있습니다.
